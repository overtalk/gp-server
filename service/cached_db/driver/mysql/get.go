...
func Get(p *Pool, document string, column []string, where Data) (Data, error) {
	...
	resp, err := p.Query(sql+columns+" FROM `"+document+"` WHERE "+strings.Join(wheres, " AND ")+" LIMIT 1", values...)
	if err != nil {
...
func GetAll(p *Pool, document string, column []string, where Data) ([]Data, error) {
	var (
		columns string
		wheres  []string
		values  []interface{}
	...
	for row.Next() {
		...
		resultData = append(resultData, assoc)
	}
